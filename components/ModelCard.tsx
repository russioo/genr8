...
export default function ModelCard({ model, selected, onSelect }: ModelCardProps) {
  ...
  return (
    <button
      onClick={() => !isComingSoon && onSelect(model.id)}
      disabled={isComingSoon}
      className={`
        w-full text-left p-4 sm:p-5 md:p-6 transition-all duration-300 min-h-[140px] sm:min-h-[150px] md:min-h-[160px] flex flex-col relative
        ${selected 
          ? 'bg-black text-white' 
    ...
    >
      {isComingSoon && (
        <div className="absolute top-3 right-3">
          <span className="text-[8px] sm:text-[9px] uppercase tracking-wider px-2 py-1 bg-[var(--accent)]/20 text-[var(--accent)] rounded-full font-semibold">
            Coming Soon
          </span>
        </div>
      )}
